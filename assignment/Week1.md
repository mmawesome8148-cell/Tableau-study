...
```
데이터가 깔끔하지 않아 바로 분석하기 어려울 때 사용함. 
데이터셋을 분석하기 좋은 형태로 바꿔줌.
```
...
```
1) 라이브: Tableau가 데이터에 직접 연결하는 방식 <br>
-> 원본 파일이 바뀌면 시트에 작업한 내용도 영향을 받음. <br>
-> 데이터 양이 많아지면 처리 속도가 늦어짐. 

2) 추출: 데이터 원본을 hyper 형태로 저장하는 것 <br>
-> 오프라인에서도 작업 가능 <br>
-> 라이브에 비해서 처리속도가 빠름. <br>
-> 새로고침을 통해 추출데이터를 업로드할 수 있음. 
```
...
### 조인: 제어, 의도적 필터링, 복제를 하는 경우 사용하는 결합 방식
```
(1) Inner 조인: 데이터를 결합할 때, 두 테이블에서 모두 일치하는 항목만 포함시킴.
(2) Left/ Right 조인: 한쪽 테이블과 다른 테이블과의 일치 항목
(3) Outer 조인: 두 테이블의 모든 값
```
...
### 혼합: 실제로 데이터를 결합하지는 않음. <br>
```
- 독립적으로 집계한 후, 한 시트에서 데이터를 시각화할 수 있도록 하는 방법 
- 하나 이상의 필드가 연결되어 있어야 함. 
- 결합을 더 추천
```
...
```
- 여러 테이블을 유니온하려면 와일드카드를 사용해야
```
> **🧞‍♀️ 유니온을 사용하기 위한 전제 조건은 무엇인가요?**

- 데이터의 테이블 구조가 동일해야 함. 
- 필드 수와 관련 필드 이름과 데이터 유형이 일치해야
...
### 라이브: 데이터 변경이 실시간으로 업데이트됨. <br>
```
- 데이터베이스에서 모든 과정이 처리됨. 
- 따라서 데이터 베이스의 성능에 따라 통합문서의 속도가 달라짐. 
```
### 추출: 원본 데이터에서 업데이트를 수신하려면 추출을 매번 새로 고쳐주어야 함. <br>
```
- 작업 시에 모든 데이터가 데이터 원본에서 태블로 서버로 저장됨. 
- 따라서 데이터 원본이 바뀌어도 시각화에 영향을 미치지 않으며 추출을 새로 고쳐야함.
- 복잡한 시각화에서 라이브보다 훨씬 속도가 빠름.
```

> **🧞‍♀️ 라이브와 추출 방법의 차이가 무엇인가요? 어떤 경우에 추출을 사용하면 좋을까요?**
- 대규모 데이터셋, 필터, 계산이 포함된 보다 복잡한 시각화에서 라이브보다 추출의 방법이 더 좋음. 
...
```
업데이트가 실시간으로 반영되는 라이브 방식을 채택해야 함. 
```
...
```
오프라인에서도 사용할 수 있는 추출방식으로 결정해야 함. 
```
