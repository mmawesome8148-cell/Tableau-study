...
```
EXCLUDE LOD는 현재 뷰에서 특정 차원을 제외하여 계산할 때 사용됨. 
-> 뷰에 있는 차원에 따라 계산하기 때문에 필터링의 영향을 받음.
FIXED는 특정 차원을 사용해 계산하므로 필터의 영향을 받지 않음. 
```
...
```
INCLUDE LOD는 현재 뷰에서 특정 차원을 추가하여 계산함
-> EXCLUDE LOD와 마찬가지로 차원 필터를 통해 값을 변경할 수 있음
```
...
```
매개변수는 동적인 값으로 변경하기 위해 활용되는 기능으로
반드시 계산식, 필터, 참조선이 함꼐 사용되어야 함. 
...
```
마크 유형 변경: 차트는 변경하지 않으면서 그래프 모양만 바꿀 수 있음
색상: 범주별로 색상을 다르게 할 수 있음.
크기: 그래프 크기 변경
레이블
```
